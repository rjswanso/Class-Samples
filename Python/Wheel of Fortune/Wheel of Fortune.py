...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        if self.prizeMoney > VOWEL_COST:
            poss_letts = [x for x in LETTERS if x not in guessed]
        else:
            poss_letts = [x for x in LETTERS if (x not in guessed) and (x not in VOWELS)]
        return poss_letts
    
    def getMove(self,category, obscuredPhrase, guessed):
        poss_letts = self.getPossibleLetters(guessed)
        if len(poss_letts) > 0:
            if self.smartCoinFlip():
                for i in range(len(self.SORTED_FREQUENCIES)):
                    if self.SORTED_FREQUENCIES[-i] in poss_letts:
                        return self.SORTED_FREQUENCIES[-i]
            else: 
                return random.choice(poss_letts)
        else: 
            return 'pass'
...
def requestPlayerMove(player, category, guessed):
    while True: # we're going to keep asking the player for a move until they give a valid one
        ...
        move = player.getMove(category, obscured_phrase, guessed)
